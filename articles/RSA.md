...
**Repairable systems** are systems that can be restored to a functioning
state after a failure. Unlike non-repairable items where we model
time-to-first-failure, repairable systems generate **recurrent events**
– a sequence of failure times over the system’s life. The
**Non-Homogeneous Poisson Process (NHPP)** provides a natural framework
for modeling these recurrence patterns.

ReliaGrowR provides four key tools for analyzing repairable systems:

1.  **[`mcf()`](https://paulgovan.github.io/ReliaGrowR/reference/mcf.md)**
    – Non-parametric Mean Cumulative Function estimation
2.  **[`exposure()`](https://paulgovan.github.io/ReliaGrowR/reference/exposure.md)**
    – Exposure analysis (total operating time at risk)
3.  **[`nhpp()`](https://paulgovan.github.io/ReliaGrowR/reference/nhpp.md)**
    – Parametric NHPP model fitting (Power Law and Log-Linear)
4.  **[`predict_nhpp()`](https://paulgovan.github.io/ReliaGrowR/reference/predict_nhpp.md)**
    – Forecasting future events from a fitted model

## Mean Cumulative Function

The **Mean Cumulative Function (MCF)** is a non-parametric estimate of
the expected cumulative number of events per system over time. It makes
...
![](RSA_files/figure-html/unnamed-chunk-3-1.png)

The step-function plot shows the estimated average cumulative events per
system over time, with dashed confidence bounds.

### Using Data Frames

The [`mcf()`](https://paulgovan.github.io/ReliaGrowR/reference/mcf.md)
...
accounts for the reduced number of systems at risk after these times.

### Exposure-Adjusted MCF

When systems are observed beyond their last event time (i.e., the system
was running but no event occurred), it is important to specify the
actual end-of-observation time. Otherwise, the MCF assumes a system left
observation at its last event, which inflates the estimated recurrence
rate.

The `end_time` parameter specifies the actual observation window per
system:
...
![](RSA_files/figure-html/unnamed-chunk-7-1.png)
...
function returns per-system end-of-observation times in its `end_times`
field, which can be passed directly to
...
**Exposure** measures the total operating time during which events can
occur across all systems in a fleet. It is fundamental to repairable
...
The
[`exposure()`](https://paulgovan.github.io/ReliaGrowR/reference/exposure.md)
function returns a table showing, at each event time, the number of
systems at risk, cumulative exposure, cumulative events, and the event
rate.
...
![](RSA_files/figure-html/unnamed-chunk-10-1.png)
...
![](RSA_files/figure-html/unnamed-chunk-12-1.png)
...
its statistical properties, while LS provides a simple regression-based
approach and supports piecewise models.
...
The **Piecewise Power Law NHPP** allows different parameters in
different time segments. This is useful when a system’s behavior changes
– for example, after a major overhaul or design change.
...
function forecasts cumulative events at future times using a fitted NHPP
model.
...
NHPP models provide a flexible framework for analyzing repairable
systems:
...
- **Piecewise models** capture changes in system behavior across
  different operational phases.
- **Forecasting** with
  [`predict_nhpp()`](https://paulgovan.github.io/ReliaGrowR/reference/predict_nhpp.md)
  supports planning for spare parts, maintenance scheduling, and
  warranty analysis.
